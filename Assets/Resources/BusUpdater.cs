...
public class BusUpdater : MonoBehaviour {
	...
	IEnumerator Start () {
		...
		//newPos = locationUpdater.getNewPosition (40.69355f, -73.97465f);
		while (!gps.gpsFix) {
			yield return null;
		}
		CallBuses ();
	...
	void Update () {
		//transform.position = Vector3.MoveTowards (transform.position, newPos, 1.0f * Time.deltaTime);
		//westboundBus.transform.position = transform.forward * Time.deltaTime;
		if (isDestroyed) {
